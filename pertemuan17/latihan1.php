<?php
    ...
    $jumlahData = count(query("SELECT * FROM supercars"));
    $jumlahHalaman = ceil($jumlahData / $jumlahDataPerhalaman);
    $halamanAktif = isset($_GET["halaman"]) ? $_GET["halaman"] : 1;
    // Logika penentuan halaman ke berapa dan jumlah data nya berapa
    $awalData = ($jumlahDataPerhalaman * $halamanAktif) - $jumlahDataPerhalaman;
    

    // Buat variabel untuk menampung query dari tabel lalu kirimkan ke function
    $supercars = query("SELECT * FROM supercars LIMIT $awalData, $jumlahDataPerhalaman");
