<?php
    ...
    // Cek cookie nya sudah ada atau belum
    if(isset($_COOKIE['unid']) && isset($_COOKIE['keyus'])) {
        $id = $_COOKIE['unid'];
        $keyus = $_COOKIE['keyus'];
        ...
        // Cek keyus
        if($keyus === hash('sha256', $row['username'])) {
            $_SESSION['login'] = true;
        }
    }
        ...
        if(mysqli_num_rows($result) === 1) {
            ...
            if(password_verify($password, $row['password'])) {
                ...
                if(isset($_POST['remember'])) {
                    // Buat cookie Ambil id dan username
                    setcookie('unid', $row['id'], time() + 60);
                    setcookie('keyus', hash('sha256', $row['username']), time() + 60);
                }
